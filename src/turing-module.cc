...
void turing_module::process(const ProcessArgs &args) {
  ...
  outputs[O_PULSE].value = std::min(
      outputs[O_GATE].value * inputs[I_CLOCK].getVoltage(), (float)10.0);
  ...
  if (!was_high && was_high != m_clock_trigger.isHigh()) {
    ...
    if (params[P_WRITE].getValue() > 0.9)
      hot = false;
    else if (mode > 0.9) {
      // leave hot alone
    } else if (mode > 0.55) {
    ...
    uint16_t mask = 0;
    size_t steps = 0;
    for (double i = 0; i < params[P_LENGTH].getValue(); i += 1) {
      mask <<= 1;
      mask |= 0x1;
      steps++;
    }
...
turing_module::turing_module() : engine::Module(), m_sequence(0) {
